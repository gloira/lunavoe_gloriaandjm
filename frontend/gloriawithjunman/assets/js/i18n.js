window.I18N = {
  en: {
    nav_home: "Home",
    nav_schedule: "Schedule",
    nav_venue: "Venue & Stay",
    nav_travel: "Travel Guide",
    nav_gallery: "Gallery",
    nav_rsvp: "RSVP",
    nav_close: "Close",
    brand_mark_main: "Gloria & Junman",
    brand_mark_sub: "Uluwatu · Bali",
    top_bar: "Gloria & Junman · Uluwatu, Bali · September 11–13, 2026",
    page_title_home: "Gloria & Junman · Bali Wedding Weekend",
    page_desc_home:
      "Join Gloria and Junman for their wedding celebration in Uluwatu, Bali on September 11-13, 2026. A weekend of love, sunset views, and dancing under the stars.",
    page_title_schedule: "Gloria & Junman · Weekend Schedule",
    page_desc_schedule:
      "View the weekend schedule for Gloria and Junman's Bali wedding. Friday arrival, Saturday ceremony, and Sunday at your own pace.",
    page_title_venue: "Gloria & Junman · Venue & Stay",
    page_desc_venue:
      "Venue and accommodation details for Gloria and Junman's wedding at Stone Villas Uluwatu, Bali.",
    page_title_travel: "Gloria & Junman · Travel Guide",
    page_desc_travel:
      "Travel guide and recommendations for visiting Uluwatu, Bali for Gloria and Junman's wedding.",
    page_title_gallery: "Gloria & Junman · Gallery",
    page_desc_gallery:
      "Photo gallery from Gloria and Junman's journey together and Bali wedding moments.",
    page_title_rsvp: "Gloria & Junman · RSVP",
    page_desc_rsvp:
      "RSVP for Gloria and Junman's Bali wedding. Let us know if you can make it!",

    hero_script: "We\u2019re getting married",
    hero_names: "Gloria &amp; Junman",
    hero_names_cn: "",
    hero_dateplace: "Uluwatu · Bali · September 11–13, 2026",
    hero_copy:
      "Join us on a cliffside in Uluwatu for a relaxed, sea-breeze weekend of vows, sunset cocktails, and dancing under the stars.",
    hero_cta_primary: "RSVP",
    hero_cta_secondary: "Weekend details",
    scroll_hint: "Scroll",

    home_intro_title: "A long weekend in Bali",
    home_intro_body:
      "Think of this as a small holiday with people we love. Arrive on Friday, settle into your villa or hotel, and ease into Bali time before the wedding on Saturday afternoon.",
    home_highlight_1:
      "Ceremony at Stone Villas Uluwatu overlooking the Indian Ocean.",
    home_highlight_2:
      "Slow mornings, coffee with a view, and optional adventures around Bali.",
    home_highlight_3:
      "No strict program on Sunday \u2014 sleep in, brunch, or continue your island journey.",
    home_story_title: "Our Story",
    home_story_body: "A glimpse of our journey together.",

    schedule_title: "Weekend schedule",
    schedule_intro:
      "We\u2019ve kept things simple so you have plenty of time to enjoy Bali on your own as well.",

    schedule_fri_label: "Friday · Settle in",
    schedule_fri_meta: "Check-in & free time",
    schedule_fri_1: "Check into your villa or hotel in Uluwatu.",
    schedule_fri_2:
      "Spend the afternoon by the pool, at a café, or exploring nearby beaches.",
    schedule_fri_3:
      "Evening at your own pace \u2014 feel free to gather with friends for dinner or sunset drinks.",

    schedule_sat_label: "Saturday · Wedding day",
    schedule_sat_meta: "Ceremony & celebration",
    schedule_sat_1:
      "15:30 · Guest arrival & check-in at Stone Villas Uluwatu.",
    schedule_sat_2: "16:00 · Cliffside ceremony.",
    schedule_sat_3: "17:00 · Sunset cocktails & photos.",
    schedule_sat_4: "19:00 · Dinner under the lights.",
    schedule_sat_5: "22:00 · After party by the pool.",
    schedule_sat_6: "01:00 · Formal events end.",

    schedule_sun_label: "Sunday · Slow goodbyes",
    schedule_sun_meta: "No formal program",
    schedule_sun_1: "Slow breakfast, pool time, or a gentle spa morning.",
    schedule_sun_2: "Head to the airport or move on to Seminyak, Canggu, etc.",
    schedule_sun_3:
      "If you stay longer, we\u2019ll share ideas for beaches, cafés, and day trips.",

    venue_title: "Venue & stay",
    venue_intro:
      "Our ceremony and reception will take place at Stone Villas Uluwatu, a cliffside villa complex with sweeping ocean views.",
    venue_story:
      "The ceremony and dinner will both be held at Stone Villas Uluwatu, a cliffside villa estate with panoramic ocean views.",
    venue_gallery_title: "Venue glimpses",

    venue_fact_1:
      "Stone Villas Uluwatu · Banjar Dinas Karang Boma, Jl. Raya Uluwatu Pecatu, Bali.",
    venue_fact_2:
      "Some guests will stay on-site at the villas, others in nearby hotels \u2014 we\u2019ll coordinate rooms within the group chat.",
    venue_fact_3:
      "The venue is on a cliff; please plan for comfortable shoes and a light layer for the evening breeze.",
    venue_note:
      "We'll confirm specific room assignments and transfer details closer to the date. If you prefer to stay off-site, we'll share a short list of hotels and villas within a 10–15 minute drive.",

    travel_title: "Travel guide",
    travel_intro:
      "Here are some ideas for how to spend your free time in Uluwatu and a few suggestions if you continue your trip to other parts of Bali.",

    travel_uluwatu_tag: "Around Uluwatu",
    travel_uluwatu_title: "Cafés, views & beach clubs",
    travel_uluwatu_body:
      "Uluwatu is all about cliffs, surf breaks, and golden sunsets. It\u2019s easy to fill a day between coffee, beaches, and a sunset drink.",
    travel_uluwatu_li1:
      "Cafés for brunch & coffee: Suka Espresso, Nourish Café, Baked, Alchemy, and The Loft.",
    travel_uluwatu_li2:
      "Sunset spots: Single Fin, Ulu Cliff House, or El Kabron for a more dressed-up evening.",
    travel_uluwatu_li3:
      "Beach clubs: Sundays Beach Club, Savaya, or other options along the Bukit peninsula for pool-and-ocean days.",
    travel_uluwatu_li4:
      "Things to do: Uluwatu Temple at sunset, Kecak fire dance, Balangan or Nyang Nyang beach viewpoints, or a surf class.",

    travel_extend_tag: "After the wedding",
    travel_extend_title: "If you keep exploring",
    travel_extend_body:
      "If you have extra days, it\u2019s easy to split your time between a few different areas with different vibes.",
    travel_extend_li1:
      "Seminyak: restaurants, boutique shopping, and rooftop sunsets. Nearby beach clubs include Potato Head, Ku De Ta, and FINNS in nearby Canggu.",
    travel_extend_li2:
      "Canggu: cafés, co-working spots, and sunset bars for a more laid-back but lively scene.",
    travel_extend_li3:
      "Ubud or the mountains: rice terraces, temples, and cooler air if you\u2019d like a contrast to the ocean.",
    travel_extend_li4:
      "If you\u2019re unsure where to stay, feel free to ask us \u2014 we\u2019re happy to help with suggestions.",

    gallery_title: "Gallery",
    gallery_intro:
      "A little moodboard of Bali and some space reserved for our photos. We\u2019ll also share a link here after the wedding where guests can upload their own pictures.",
    gallery_after_note:
      "After the wedding, we'll share a link where everyone can upload and download photos from the celebration.",

    rsvp_title: "RSVP",
    rsvp_intro:
      "Because space at the venue is limited, please help us by responding as accurately as you can. You\u2019ll be able to update us later if plans change.",
    rsvp_step1: "Step 1 · Guest access",
    rsvp_step2: "Step 2 · Your details",
    rsvp_password_label: "Wedding password",
    rsvp_password_hint:
      "You\u2019ll find this on your invitation. Please keep it within close friends & family only.",
    rsvp_password_placeholder: "Enter the password from your invitation",
    rsvp_name_label: "Full name",
    rsvp_name_placeholder: "Your full name",
    rsvp_attend_label: "Will you attend?",
    rsvp_attend_yes: "Yes, I\u2019ll be there",
    rsvp_attend_no: "Sadly I can\u2019t make it",
    rsvp_message_label: "Notes (dietary, plus one, etc.)",
    rsvp_message_placeholder:
      "Dietary needs, plus one name, or anything you’d like us to know.",
    rsvp_btn_unlock: "Unlock RSVP form",
    rsvp_btn_submit: "Send RSVP",

    rsvp_status_pwd_missing: "Please enter the wedding password.",
    rsvp_status_pwd_wrong:
      "That password doesn\u2019t look right. Please check your invitation.",
    rsvp_status_sending: "Sending your RSVP…",
    rsvp_status_missing:
      "Please fill in your name and whether you will attend.",
    rsvp_status_success: "Thank you! We’ve received your RSVP 🥂",
    rsvp_status_error:
      "Sorry, something went wrong. Please try again later or contact us directly.",

    footer_text: "Gloria & Junman · Bali · 2026 · Lunavoe",
  },

  zh: {
    nav_home: "首页",
    nav_schedule: "行程安排",
    nav_venue: "婚礼场地与住宿",
    nav_travel: "旅行指南",
    nav_gallery: "照片集",
    nav_rsvp: "出席回覆",
    nav_close: "关闭",
    brand_mark_main: "雨言和俊文",
    brand_mark_sub: "乌鲁瓦图 · 巴厘岛",
    top_bar: "雨言和俊文 · 乌鲁瓦图 · 巴厘岛 · 2026年9月11日-13日",
    page_title_home: "雨言和俊文 · 巴厘岛婚礼周末",
    page_desc_home:
      "欢迎参加雨言和俊文在巴厘岛乌鲁瓦图的婚礼（2026年9月11-13日）。一个关于爱、落日与星空舞会的周末。",
    page_title_schedule: "雨言和俊文 · 周末行程",
    page_desc_schedule:
      "查看我们巴厘岛婚礼的周末行程：周五抵达、周六婚礼、周日自由安排。",
    page_title_venue: "雨言和俊文 · 场地与住宿",
    page_desc_venue:
      "婚礼场地与住宿信息：巴厘岛乌鲁瓦图 Stone Villas。",
    page_title_travel: "雨言和俊文 · 出行指南",
    page_desc_travel:
      "乌鲁瓦图巴厘岛出行与游玩建议，供参加婚礼的亲友参考。",
    page_title_gallery: "雨言和俊文 · 照片集",
    page_desc_gallery: "记录我们与巴厘岛婚礼时刻的照片集。",
    page_title_rsvp: "雨言和俊文 · 出席回覆",
    page_desc_rsvp: "请填写出席回覆，告诉我们你是否能来参加婚礼。",

    hero_script: "我们要结婚啦",
    hero_names: "Gloria &amp; Junman",
    hero_names_cn: "雨言 · 俊文",
    hero_dateplace: "乌鲁瓦图 · 巴厘岛 · 2026年9月11日-13日",
    hero_copy:
      "欢迎你来到巴厘岛乌鲁瓦图的海边，与我们一起在悬崖之上、日落和海风中见证这一刻。",
    hero_cta_primary: "填写出席回覆",
    hero_cta_secondary: "查看周末安排",
    scroll_hint: "向下滚动",

    home_intro_title: "一个小小的巴厘岛假期",
    home_intro_body:
      "把这次婚礼当作一次和亲友一起度假的周末吧。周五抵达入住酒店或别墅，慢慢进入“巴厘岛时间”，周六下午参加婚礼，周日自由活动。",
    home_highlight_1: "婚礼地点为 Stone Villas Uluwatu 悬崖别墅，可眺望整片印度洋。",
    home_highlight_2: "轻松的早晨、咖啡与海景，还有很多自由探索巴厘岛的时间。",
    home_highlight_3: "周日不设正式活动，可以睡到自然醒、吃早午餐，或继续下一段行程。",
    home_story_title: "我们的故事",
    home_story_body: "一起走过的点滴剪影。",

    schedule_title: "周末行程",
    schedule_intro: "我们希望大家在巴厘岛可以放松，所以正式安排不多，留出足够的自由时间。",

    schedule_fri_label: "周五 · 抵达与入住",
    schedule_fri_meta: "入住 & 自由活动",
    schedule_fri_1: "抵达巴厘岛后前往乌鲁瓦图入住酒店或别墅。",
    schedule_fri_2: "下午可以在泳池边、咖啡馆或附近沙滩放松。",
    schedule_fri_3: "晚上自由安排，欢迎和朋友约好一起吃饭或看日落。",

    schedule_sat_label: "周六 · 婚礼当天",
    schedule_sat_meta: "仪式 & 晚宴",
    schedule_sat_1: "15:30 · 抵达 Stone Villas Uluwatu 签到入场。",
    schedule_sat_2: "16:00 · 悬崖婚礼仪式。",
    schedule_sat_3: "17:00 · 日落鸡尾酒 & 合影时间。",
    schedule_sat_4: "19:00 · 户外灯光晚宴。",
    schedule_sat_5: "22:00 · 泳池旁 After Party。",
    schedule_sat_6: "01:00 · 正式活动结束。",

    schedule_sun_label: "周日 · 慢慢道别",
    schedule_sun_meta: "无正式安排",
    schedule_sun_1: "可以睡到自然醒，安排早午餐、Spa 或继续在泳池边放空。",
    schedule_sun_2: "根据行程前往机场，或前往水明漾、仓古等地继续度假。",
    schedule_sun_3: "如果你打算多待几天，我们也会在页面里分享一些推荐路线。",

    venue_title: "婚礼场地与住宿",
    venue_intro:
      "婚礼仪式与晚宴都会在 Stone Villas Uluwatu 举行，这是一处位于悬崖上的独栋别墅群，可以俯瞰整片海景。",
    venue_story:
      "婚礼仪式与晚宴都会在 Stone Villas Uluwatu 举行，这是一处位于悬崖上的独栋别墅群，可以俯瞰整片海景。",
    venue_gallery_title: "场地掠影",

    venue_fact_1:
      "Stone Villas Uluwatu · Banjar Dinas Karang Boma, Jl. Raya Uluwatu Pecatu, Bali。",
    venue_fact_2:
      "部分亲友会入住场地别墅，其他宾客会安排在附近酒店，具体房间我们会在群里统一协调。",
    venue_fact_3: "场地位于悬崖边，建议穿着舒适好走的鞋，并准备一件薄外套以防晚上海风。",
    venue_note:
      "临近日期我们会确认具体房型分配和接送安排。如果你更想住在场地外，我们会分享一份车程约10-15分钟内的酒店与别墅清单。",

    travel_title: "旅行指南",
    travel_intro:
      "这里整理了一些乌鲁瓦图周边的咖啡馆、餐厅与海滩推荐，以及如果你计划在婚礼后继续在巴厘岛旅行，可以考虑的几个区域。",

    travel_uluwatu_tag: "乌鲁瓦图周边",
    travel_uluwatu_title: "咖啡馆 · 景观餐厅 · 海滩俱乐部",
    travel_uluwatu_body:
      "乌鲁瓦图的关键词是悬崖、海浪和日落。你可以在咖啡馆、沙滩和日落酒吧之间度过非常充实的一天。",
    travel_uluwatu_li1:
      "早午餐 & 咖啡：Suka Espresso、Nourish Café、Baked、Alchemy、The Loft 等。",
    travel_uluwatu_li2:
      "日落观景：Single Fin、Ulu Cliff House 或 El Kabron（稍微正式一点的晚间氛围）。",
    travel_uluwatu_li3:
      "海滩俱乐部：Sundays Beach Club、Savaya 等，适合白天泡在泳池和海边。",
    travel_uluwatu_li4:
      "推荐活动：傍晚在乌鲁瓦图寺看日落和 Kecak 火舞、Balangan 或 Nyang Nyang 观景点、学习冲浪等。",

    travel_extend_tag: "婚礼之后",
    travel_extend_title: "如果你打算继续玩",
    travel_extend_body:
      "如果还有额外几天行程，可以把时间分配在不同区域，体验不一样的巴厘岛氛围。",
    travel_extend_li1:
      "水明漾（Seminyak）：餐厅、精品店和 Rooftop 日落酒吧集中地，附近有 Potato Head、Ku De Ta、以及稍远一点仓古的 FINNS 等海滩俱乐部。",
    travel_extend_li2:
      "仓古（Canggu）：咖啡馆、冲浪和日落酒吧的聚集地，氛围更 laid-back 但也很热闹。",
    travel_extend_li3:
      "乌布或山间区域：梯田、寺庙与山林，适合想换个完全不同氛围、远离海边的几天。",
    travel_extend_li4:
      "如果不知道住哪里或者怎么安排路线，也可以随时和我们说，我们很乐意一起帮你规划。",

    gallery_title: "照片集",
    gallery_intro:
      "这里先放一些巴厘岛氛围图和占位图，之后会更新我们的婚纱照与婚礼当天照片，也会提供链接供大家上传自己的照片。",
    gallery_after_note: "婚礼结束后，我们会分享一个链接，方便大家上传和下载当天的照片。",

    rsvp_title: "出席回覆（RSVP）",
    rsvp_intro:
      "场地名额有限，请尽量如实填写是否出席；如果之后计划有变，也欢迎再和我们联系更新。",
    rsvp_step1: "第一步 · 宾客验证",
    rsvp_step2: "第二步 · 填写信息",
    rsvp_password_label: "婚礼密码",
    rsvp_password_hint: "密码会写在你的邀请卡上，请不要在熟人圈以外传播。",
    rsvp_password_placeholder: "请输入邀请卡上的密码",
    rsvp_name_label: "姓名",
    rsvp_name_placeholder: "你的姓名",
    rsvp_attend_label: "是否出席？",
    rsvp_attend_yes: "会出席",
    rsvp_attend_no: "无法前来",
    rsvp_message_label: "备注（饮食、是否携伴等）",
    rsvp_message_placeholder: "如有饮食禁忌、Plus One 姓名、或想对我们说的话，可以写在这里。",
    rsvp_btn_unlock: "进入出席回覆表单",
    rsvp_btn_submit: "提交",

    rsvp_status_pwd_missing: "请先输入婚礼密码。",
    rsvp_status_pwd_wrong: "密码似乎不正确，请再确认一下邀请卡上的内容。",
    rsvp_status_sending: "正在发送出席回覆……",
    rsvp_status_missing: "请填写姓名，并选择是否出席。",
    rsvp_status_success: "谢谢！我们已经收到你的出席回覆 🥂",
    rsvp_status_error: "抱歉，提交失败，请稍后重试，或直接联系我们。",

    footer_text: "雨言和俊文 · 巴厘岛 · 2026 · Lunavoe",
  },
};
